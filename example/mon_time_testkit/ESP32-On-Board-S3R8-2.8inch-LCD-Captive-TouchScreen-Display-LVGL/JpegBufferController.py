...
class ResourceBuffer:
    ...
    def __init__(self, name, root_path, config, decoder, strict_mode=False):
        ...
        # === 三重緩衝 ===
        self.buffer_0 = bytearray(self.buffer_size)
        self.buffer_1 = bytearray(self.buffer_size)
        self.buffer_2 = bytearray(self.buffer_size)
        ...
        self.current_frame = 0      # 當前顯示的幀
        self.decoding_frame = -1    # 正在解碼的幀
        self.target_frame = -1      # 目標跳轉幀 (用於去重)
    ...
    def force_jump_to_frame(self, frame_index):
        """
        強制同步跳轉 (阻塞版本,僅在必要時使用)
    ...
    def jump_to_frame(self, frame_index):
        ...
        優化策略:
        1. 如果已經在顯示目標幀,立即返回 (零開銷)
        2. 如果目標幀正在解碼,等待完成後返回 (避免重複解碼)
        3. 如果是新請求,取消當前解碼並啟動新解碼
        
        Returns:
            當前可用的 FrameBuffer (可能不是目標幀,但保證無阻塞)
        """
        # 參數驗證
        if not (0 <= frame_index <= self.depth):
            return self.get_current_buffer()
        ...
        # === 去重檢查 2: 目標幀正在解碼中 ===
        if self.decoding_frame == frame_index and self.decode_thread_running:
            # 檢查是否解碼完成
            if self.ready_idx != -1 and self.decoding_frame == frame_index:
                # 解碼完成,執行切換
                self._swap_to_ready()
            # 無論是否完成,直接返回當前緩衝 (非阻塞)
            return self.get_current_buffer()
        ...
        if self.decode_thread_running:
            # 如果正在解碼,使用第三個緩衝
            busy_buffers = {self.display_idx}
            if self.decoding_idx != -1:
                busy_buffers.add(self.decoding_idx)
            idle_idx = ({0, 1, 2} - busy_buffers).pop()
        else:
            # 沒有解碼,使用非顯示緩衝
            idle_idx = 1 if self.display_idx != 1 else 2
        
        # 啟動解碼
        self._start_background_decode(frame_index, buffer_idx=idle_idx)
        
        # 立即返回當前緩衝 (非阻塞)
        return self.get_current_buffer()
    
    def _swap_to_ready(self):
        """內部方法: 切換到準備好的緩衝"""
        if self.ready_idx == -1:
            return
...
class JpegBufferController:
    ...
    def jump_to_frame(self, resource_name, frame_index):
        ...
        Returns:
            當前可用的 FrameBuffer (可能不是目標幀)
        """
        if resource_name in self.resources:
            return self.resources[resource_name].jump_to_frame(frame_index)
        return None
    
    def force_jump_to_frame(self, resource_name, frame_index):
        """
        強制同步跳轉 (阻塞版本)
        ...
        """
        if resource_name in self.resources:
            return self.resources[resource_name].force_jump_to_frame(frame_index)
        return None
